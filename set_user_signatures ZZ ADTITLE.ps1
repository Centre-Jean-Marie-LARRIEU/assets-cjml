# set_user_signatures.ps1 (v50.01 - Ajout de la mise à jour AD Title)
#
param(
...
SYNTAXE:
    .\set_user_signatures.ps1 [-SingleUserEmail <string>] [-IncludeSuspended] [-AddDigitalCard] [-GeneratePrintQr] [-GeneratePrintableCard] [-GeneratePdfCard] [-ShowHelp] [-DebugMode] [-CleanInactiveCards] [-UpdateAdTitle]
...
    -UpdateAdTitle
        Commutateur. Si présent, le script mettra à jour l'attribut 'Title' de l'utilisateur dans l'Active Directory.
        Nécessite que le script soit exécuté avec des droits suffisants sur l'AD et que le module ActiveDirectory soit installé.
...
    # Met à jour l'attribut 'Title' de l'utilisateur dans l'Active Directory à partir de GAM.
    .\set_user_signatures.ps1 -UpdateAdTitle -SingleUserEmail "s.gille@cjml.fr"
"@
    Write-Host $helpText
    return
}
...
$usersToProcess = @()
if (-not $ShowHelp) {
    # Le champ "organizations" est crucial pour le titre et le département, à récupérer si on veut faire l'AD Sync
...
    
    # NOUVEAU BLOC pour la mise à jour de l'AD
    if ($UpdateAdTitle) {
